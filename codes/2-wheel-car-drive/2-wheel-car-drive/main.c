 ...
 * 2-wheel-car-drive.c
 *
 * Created: 04-01-2018 15:43:34
 * Author : Sahil
 *
 * Description:
 *   Control system for a 4-wheel car drive robot
...
#define drive_pwm1 OCR3A    // Right Wheel
#define drive_pwm2 OCR3B    // Left Wheel
...
void manual();
void drive_2W(int y_vect, int w_vect);
...
int main(void)
{
    ...
    DDRE = 0xFF;    // PWM outputs (OC3A, OC3B)
    PORTE = 0xFF;   // Pull-up
    ...
    drive_2W(0, 0, 0);  // Stop wheels on start
...
void manual()
{
    drive_2W(xj1, (xj2 + yj2) / 2);  // yj1 = forward/backward, (xj2 + yj2)/2 = rotation
}

// Differential (car-style) 2-wheel drive
void drive_2W(int y_vect, int w_vect)
{
    w1 = y_vect - w_vect;
    w2 = y_vect + w_vect;
...
// Initialize PWM for Timer3 (OC3A, OC3B)
void pwm_init()
{
