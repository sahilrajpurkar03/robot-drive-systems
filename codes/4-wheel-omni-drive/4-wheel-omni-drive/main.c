/*
 * 4-wheel-omni-drive.c
 *
 * Created: 14-02-2018 14:45:23
 * Author : Sahil
 *
 * Description:
 *   Control system for a 4-wheel omni-directional drive robot
...
#define drive_pwm1 OCR3A    // Front Right Wheel
#define drive_pwm2 OCR3B    // Front Left Wheel
#define drive_pwm3 OCR3C    // Rear Right Wheel
#define drive_pwm4 OCR4A    // Rear Left Wheel
...
int main(void)
{
    ...
    drive_4W(0, 0, 0);  // Stop wheels on start
...
void manual()
{
    drive_4W(xj1, yj1, (xj2 + yj2) / 2);  // Drive via joystick input
}

// Omni-wheel inverse kinematics
void drive_4W(int x_vect, int y_vect, int m_vect)
{
    w1 = -x_vect - y_vect + m_vect;
    w2 = x_vect - y_vect - m_vect;
    w3 = x_vect - y_vect + m_vect;
    w4 = -x_vect - y_vect - m_vect;
...
// Initialize PWM for Timer3 (OC3A, OC3B, OC3C, OC4A)
void pwm_init()
{
    ...
    TCCR4B |= (1 << CS42) | (1 << CS40) | (1 << WGM42);  // Fast PWM with prescaler 1024
}
